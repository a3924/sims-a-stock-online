...
(function (global) {
  ...
  // 从任意粘贴文本中挑出候选种子串（纯 base62 且 ≥8 位，天然跳过中文与标点噪声）
  function seedCandidates(text) {
    var out = [], m, re = /[0-9A-Za-z]{8,}/g;
    var s = String(text || '');
    while ((m = re.exec(s)) !== null) out.push(m[0]);
    return out;
  }
  // 解析单段种子串（严格位校验），返回 {codes,startIdx,payload} 或 {err}
  ...
  // 解析种子输入：自动从「整段分享文案」中识别出种子（容忍前缀/空白/换行/前后其它文字，
  // 每段候选逐一做位校验，取第一个通过者；r32 起支持把灯泡/结算复制出的整段文案直接粘回）
  function decodeSeed(text) {
    var cands = seedCandidates(text);
    if (!cands.length) return { err: '没找到种子编码，请确认复制完整（或以 #我的人生模拟器·A股版 开头）' };
    var first = null;
    for (var i = 0; i < cands.length; i++) {
      var d = parseSeedPayload(cands[i]);
      if (!d.err) return d;
      if (!first) first = d;
    }
    return first;   // 候选全校验不过 → 报告第一个候选的具体原因（截断/被改动等）
  }
  ...
  function renderSeedPreview(text) {
    ...
    if (!box) return;
    if (!String(text || '').trim()) { box.innerHTML = '<span class="seed-dim">粘贴种子（或对方发来的整段分享文案），这里会显示这局有几只股票、从哪一天开始。</span>'; return; }
    var d = decodeSeed(text);
  ...
  function toggleMulti() {
    ...
    if (multiOn) {
      ...
      // 手机：1+1+1+1 纵排、页面随图变长；买卖栏 order5 流式落在 4 图之后最下（不遮挡），桌面：flex 双列网格
      el('multi-panel').style.display = 'flex';
      el('multi-panel').classList.add('with-trade');
      el('btn-compare').classList.add('on');
      var sg = el('screen-game'); if (sg) sg.classList.toggle('multi-on', isNarrow());   // r34：手机多图加 class，CSS 解除高度压缩使买卖栏落 4 图之后
      renderMulti();
    } else {
      el('chart-wrap').style.display = 'flex';
      ...
      var sg0 = el('screen-game'); if (sg0) sg0.classList.remove('multi-on');   // r34：退出多图恢复正常纵向布局
      layout();
      renderGame();
    }
  }
